...
class AcercamientosService {
  async crear(acercamiento) {
    const id = uuidv4();
    ...
    const params = {
      TableName: TABLES.ACERCAMIENTOS,
      Item: item,
    };
  ...
  async actualizar(id, acercamiento) {
    const timestamp = new Date().toISOString();
    ...
    Object.keys(acercamiento).forEach((key) => {
      updateExpression.push(`#${key} = :${key}`);
      expressionAttributeNames[`#${key}`] = key;
      expressionAttributeValues[`:${key}`] = acercamiento[key];
    });
  ...
  async eliminar(id) {
    ...
    return { message: 'Acercamiento eliminado correctamente' };
  }

  async obtenerEstadisticas(filtros = {}) {
    const acercamientos = await this.listar(filtros);

    // Total de acercamientos
    const totalacercamientos = acercamientos.length;

    // Acercamientos del mes actual
    const mesActual = new Date().getMonth();
    const añoActual = new Date().getFullYear();
    const acercamientosMes = acercamientos.filter(c => {
      const fecha = new Date(c.fecha);
      return fecha.getMonth() === mesActual && fecha.getFullYear() === añoActual;
    }).length;

    // Estados de animo más frecuentes
    const estadosMap = {};
    acercamientos.forEach(c => {
      const estados = Array.isArray(c.estadosAnimo) ? c.estadosAnimo : [];
      estados.forEach(estado => {
        estadosMap[estado] = (estadosMap[estado] || 0) + 1;
      });
    });
    const motivosMasFrecuentes = Object.entries(estadosMap)
    ...
    const acercamientosPorFecha = Object.entries(fechasMap)
    ...
    return {
      ...
      lugaresTrabajo,
      acercamientosPorFecha,
    };
  }
}

module.exports = new AcercamientosService();
